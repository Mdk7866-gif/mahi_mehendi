      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
        <div className="flex justify-between items-center h-14 sm:h-16">
          ...
          <div className="hidden md:flex space-x-4 lg:space-x-8 items-center">
            {['/gallery', '/reviews', '/about', '/contact', '/admin'].map((href) => (
              <Link 
                key={href} 
                href={href} 
          ...
          <button className="md:hidden text-[#6D4C41] shrink-0 ml-2" onClick={() => setIsOpen(!isOpen)} aria-label="Menu">
        ...
        {isOpen && (
          ...
          >
            {['Home', 'Gallery', 'Reviews', 'About', 'Contact', 'Admin'].map((item, i) => (
              <Link
                key={item}
